...
\end{aligned}
$$
由 $\frac{p _ n}{\sum p _ i} \to 0(n\to\infty)$ 知，$\exists N _ 2,\forall n > N _ 2,\frac{p _ n}{\sum p _ i} < \frac{\epsilon}{2\zeta(N _ 1-1)}$，而同时 $\forall m \le n,\frac{p _ m}{p _ 1+\dots+p _ m} \ge \frac{p _ m}{p _ 1+\dots+p _ m+\dots+p _ n}$，故记 $N=\max(N _ 1+N _ 2)$，那么 $\forall n > N$ 有

$$
\begin{aligned}
\frac{\epsilon}{2} + \frac{\zeta(p _ {n-N_1+2}+ \dots + p _ n)}{p _ 1+p _ 2+\dots+p _ n} \le \frac{\epsilon}{2}+\zeta\sum _ {i=n-N _ 1+2} ^ {n} \frac{p _ i}{\sum _ {j=1} ^ {i}p _ j}\le \frac{\epsilon}{2} + \zeta (N _ 1-1) \times \frac{\epsilon}{2\zeta(N _ 1-1)}=\epsilon
\end{aligned}
$$
