...
假设定理的结论不成立，即 $E$ 不存在上确界，那么，设 $a _ i \in E,a _ {i+1} > a _ i$，由 $E$ 不存在最大值可知这样的 $a _ i$ 始终存在。
同时设 $E$ 的上界集合为 $B$，易知 $B$ 不存在最小值。设 $b _ i \in B,b _ {i+1} < b _ i$，由 $E$ 无上确界知这样的 $b _ i$ 始终存在。
